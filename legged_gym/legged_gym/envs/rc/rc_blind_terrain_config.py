...
class RCCfg(LeggedRobotCfg):
    ...
    class terrain(LeggedRobotCfg.terrain):
        ...
        terrain_length = 12. # 单块子地形长度，单位米；决定每个坡、楼梯、高台区块在前进方向上的可用距离
        terrain_width = 12. # 单块子地形宽度，单位米；决定每个侧坡、踏块区、坑洞区在横向上的覆盖范围
        num_rows= 15 # 地形难度层数；行数越多，课程学习的难度等级越细，最高难地形也会覆盖到更多层级，决定最高等级上限
        num_cols = 30 # 地形类型列数；列数越多，同一难度下可并行生成的坡、楼梯、高台、踏块、坑洞等类型越丰富
    ...
    class commands(LeggedRobotCfg.commands):
        ...
        max_curriculum = 2.0 # 指令课程学习的最大放宽尺度
        ...
        class ranges( LeggedRobotCfg.commands.ranges):
            ...
            lin_vel_y = [-1.0, 1.0]   # 横向线速度命令范围，单位 m/s
            ang_vel_yaw = [-3.14, 3.14]    # 偏航角速度命令范围，单位 rad/s
    ...
    class domain_rand(LeggedRobotCfg.domain_rand):
        ...
        payload_mass_range = [-2, 2]  # 额外负载质量随机范围，单位 kg
        ...
        link_mass_range = [0.8, 1.2]  # 连杆质量缩放范围；1.0 表示原始质量
        
        randomize_friction = True  # 是否随机化摩擦系数
        friction_range = [0.2, 1.25]  # 摩擦系数随机范围
        
        randomize_restitution = True  # 是否随机化恢复系数
    ...
    class rewards(LeggedRobotCfg.rewards):
        class scales(LeggedRobotCfg.rewards.scales):
            ...
            tracking_lin_vel = 1.5  # 线速度跟踪奖励权重；鼓励机体跟踪给定 x/y 速度命令
            tracking_ang_vel = 1.0  # 角速度跟踪奖励权重；鼓励机体跟踪给定 yaw 角速度命令
            lin_vel_z = -2.2  # z 方向线速度惩罚权重；抑制机体上下乱跳
            ang_vel_xy = -0.08  # x/y 方向角速度惩罚权重；抑制滚转和俯仰角速度过大
            orientation = -2.0  # 姿态惩罚权重；鼓励机身保持接近平衡姿态
            dof_acc = -2.5e-7  # 关节加速度惩罚权重；抑制关节剧烈加减速
            joint_power = 0.0  # 关节功率项权重；当前为 0，表示不启用功率约束
            base_height = -15.0  # 机身高度惩罚权重；鼓励机身高度接近目标值
            foot_clearance = -0.15  # 足端净空高度相关项权重；当前实现是对偏离目标高度的运动进行惩罚
            action_rate = -0.01  # 动作变化率惩罚权重；抑制相邻两步动作变化过大
            smoothness = -0.007  # 二阶平滑惩罚权重；抑制动作序列出现明显拐点和高频抖动
            feet_air_time = 1.5  # 足端腾空时间奖励权重；鼓励形成更清晰的摆动相
            collision = -0.1  # 碰撞惩罚权重；惩罚不希望发生接触的刚体碰撞
            stumble = -0.01  # 绊腿惩罚权重；惩罚脚撞到近似垂直障碍物
            stand_still = -0.01 # 静止站立惩罚权重；当前为 0，不启用该项
            torques = -0.00006  # 力矩惩罚权重；当前为 0，不约束关节输出力矩大小
            ...
            torque_limits = -0.01  # 力矩限位惩罚权重；惩罚逼近力矩极限
            hip_abduction_deviation = -1.2  # 髋外展/内收关节偏离默认姿态惩罚权重；用于约束横向开腿过大
    ...
    class noise(LeggedRobotCfg.noise):
        ...
        noise_level = 1.0 # 全局噪声缩放系数；会统一放大或缩小各类噪声幅度
        class noise_scales(LeggedRobotCfg.noise.noise_scales):
            dof_pos = 0.01  # 关节位置观测噪声幅度
            dof_vel = 1.5  # 关节速度观测噪声幅度
            lin_vel = 0.1  # 线速度观测噪声幅度
            ang_vel = 0.2  # 角速度观测噪声幅度
...
# PPO 训练配置
class RCCfgPPO(LeggedRobotCfgPPO):
